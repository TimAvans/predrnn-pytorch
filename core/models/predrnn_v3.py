...
class RNN(nn.Module):
    def __init__(self, num_layers, num_hidden, configs):
        ...
        self.cross_layer_attn = nn.MultiheadAttention(embed_dim=num_hidden[i], num_heads=1, batch_first=True)
    ...
    def apply_cross_layer_attention(self, h_t):
        # h_t: list of [B, C, H, W] for each layer
        h_stack = torch.stack(h_t, dim=1)  # [B, L, C, H, W]
        B, L, C, H, W = h_stack.shape
        h_flat = h_stack.permute(0, 3, 4, 1, 2).reshape(B * H * W, L, C)  # [BHW, L, C]

        attn_out, _ = self.cross_layer_attn(h_flat, h_flat, h_flat)  # [BHW, L, C]
        attn_out = attn_out.mean(dim=1)  # or use [:, -1]
        attn_out = attn_out.view(B, H, W, C).permute(0, 3, 1, 2)  # [B, C, H, W]
        return attn_out
    ...
    def forward(self, frames_tensor, mask_true):
        ...
        attn_out = self.apply_cross_layer_attention(h_t)
        h_t[0] = h_t[0] + attn_out  # of h_t[-1] = h_t[-1] + attn_out
