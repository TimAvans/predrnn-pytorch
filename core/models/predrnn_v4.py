...
from core.utils.gradient_loss import gradient_loss
...
class RNN(nn.Module):
    ...
    def forward(self, frames_tensor, mask_true):
        ...
        lambda_gdl = 0.06 # Hyperparameter for GDL loss
        pixel_loss = self.MSE_criterion(next_frames, frames_tensor[:, 1:])

        # Reshape to [B*T, C, H, W] for GDL
        B, T, H, W, C = next_frames.shape
        pred = next_frames.permute(0, 1, 4, 2, 3).reshape(B*T, C, H, W)
        gt = frames_tensor[:, 1:].permute(0, 1, 4, 2, 3).reshape(B*T, C, H, W)

        gdl = gradient_loss(pred, gt)
